...
class TextTools:
    ...
    @staticmethod
    def remove_building_equip_text(display_name_string):
        """
        Splits the string on "-" and returns the first match.  Effective in removing the "-buildingEquip" text
        from a string.  If a "-" is not present, it will return the original string.
        Example: "Canal Place-buildingEquip" → "Canal Place"
    ...
    @staticmethod
    def remove_type_information_text(type_text):
        """
        Removes the type text from an entry.  This text usually appears as "n:", or something similar.
        For example: "r:2f279255-a859-4a2d-974e-1dfcb5f0c627" where the "r:" needs to be removed.
        ...
        if not type_text:
            raise Exception("Argument cannot be None.")
        if type_text[1] != ":":
            raise Exception("Argument has the wrong format.")
        if len(type_text) != 38:
            raise Exception("Argument should be 38 characters.")
        return type_text[2:]


    @staticmethod
    def get_float_from_string(float_text):
        """
    ...
    @staticmethod
    def get_query_string(reference_target, reference_string, item_target, item_tag_list):
        """
        ...
        """
        if not reference_target or not reference_string or not item_target or not item_tag_list:
